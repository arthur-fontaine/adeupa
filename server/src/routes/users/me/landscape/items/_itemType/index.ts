...
const landscapeItem: FastifyPluginAsync = async (fastify, opts): Promise<void> => {
    ...
    fastify.get(
        ...
        if (itemType !== 'LandscapeSkyColor' && itemType !== 'LandscapeDistrict') {
            return reply.badRequest(
            'itemType should be equal to "LandscapeSkyColor" or "LandscapeDistrict"'
            )
        }
        ...
        const items = await prisma.item.findMany({
            ...
            type: itemType === 'LandscapeSkyColor' ? 'LandscapeSkyColor' : 'LandscapeDistrict',
            },
            include: {
            requiredBadges: true,
            },
    ...
    fastify.put(
        ...
        await prisma.character.update({
            ...
            data: {
            enabledItems: {
                disconnect: { id: currentSelectedItem?.id },
